				...
				for (int y = 0; y < var.getValues().getDimY(); y++) {
					...
					for (int x = 0; x < var.getValues().getDimX(); x++) {
						...
						if(idx < 0)
						{
							...
							if (y * x == 0) {
								printWriter.println("<th align=center>" + var.getValues().getValue(y, x).substring(0, idx) + " | " + "<font color=red>" + var.getValues().getValue(y, x).substring(idx+1) + "</font>" + "</th>");
							} else {
								printWriter.println("<td align=center>" + var.getValues().getValue(y, x).substring(0, idx) + " | " + "<font color=red>" + var.getValues().getValue(y, x).substring(idx+1) + "</font>" + "</td>");
							}
						}
					}
