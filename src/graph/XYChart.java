...
public final class XYChart extends JPanel {
	...
	private void createXYLine(Curve curve) {
		...
		this.add(new PanelXYChart(new PanelXYPlot("Y = f(X)", "X [" + curve.getUnitX() + "]",
				"Y [" + curve.getUnitZ() + "]", seriesCollection, true), PanelXYChart.RIGHT_POSITION, false));
	}
	...
	private void createIsoX(Map map) {
		...
		this.add(new PanelXYChart(new PanelXYPlot("Z = f(Y)", "Y [" + map.getUnitY() + "]", "Z [" + map.getUnitZ() + "]",
				seriesCollection, true), PanelXYChart.RIGHT_POSITION, true));
	}
	...
	private void createIsoY(Map map) {
		...
		for (short y = 0; y < map.getDimY() - 1; y++) {
			...
			serie = new Serie(map.getyValues()[y]);

			for (short x = 0; x < map.getDimX() - 1; x++) {
				try {
					serie.addPoint(Double.parseDouble(map.getxValues()[x]), Double.parseDouble(map.getzValue(y, x)));
				} catch (NumberFormatException nfe) {
					if (Utilitaire.isNumber(map.getzValue(y, x))) {
						serie.addPoint(x, Double.parseDouble(map.getzValue(y, x)));
					} else {
