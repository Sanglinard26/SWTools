...
/**
 * A InfiniteProgressPanel-like component, but more efficient. This is the preferred class to use unless you need the total control over the
 * appearance that InfiniteProgressPanel gives you.<br />
 * <br />
 ...
 * drawn upon a white veil which alpha level (or shield value) lets the underlying component shine through. This panel is meant to be used as a
 * <i>glass pane</i> in the window performing the long operation. <br />
 * <br />
 * Calling setVisible(true) makes the component visible and starts the animation. Calling setVisible(false) halts the animation and makes the
 * component invisible. Once you've started the animation all the mouse events are intercepted by this panel, preventing them from being forwared to
 * the underlying components. <br />
 * <br />
 * The panel can be controlled by the <code>setVisible()</code>, method. <br />
 * <br />
 * This version of the infinite progress panel does not display any fade in/out when the animation is started/stopped.<br />
 * <br />
 * Example: <br />
 * <br />
 * 
...
public final class SingleComponentInfiniteProgress extends JComponent {
    ...
    public SingleComponentInfiniteProgress(boolean i_bUseBackBuffer, int numBars) {
        this(i_bUseBackBuffer, numBars, DEFAULT_FPS, NO_AUTOMATIC_RESIZING, NO_MAX_BAR_SIZE);
    }

    public SingleComponentInfiniteProgress(boolean i_bUseBackBuffer, int numBars, int fps, double resizeRatio, double maxBarSize) {
        ...
        // create colors
        /*
         * for(int i = 0; i < bars.length; i++) { int channel = 224 - 128 / (i + 1); colors[i] = new Color(channel, channel, channel); colors[numBars
         * + i] = colors[i]; }
         */
        // END CHANGE */
        // set cursor
    ...
    protected void calcBarsForBounds(final int width, final int height, final boolean honorMinBarSize) {
        // update centering transform
        centerAndScaleTransform = new AffineTransform();
        centerAndScaleTransform.translate((double) width / 2d, (double) height / 2d);
    ...
    protected double paintBars(final Graphics g, final boolean paintBackground) {
        ...
        if (paintBackground) {
            if (imageBuf != null) {
                ...
                g.drawImage(imageBuf, oClip.x, oClip.y, oClip.x + oClip.width, oClip.y + oClip.height, oClip.x, oClip.y, oClip.x + oClip.width,
                        oClip.y + oClip.height, null);
    ...
    public static double getTextMaxY(final String text, final Font font, final Graphics2D g2, final double y) {
        final TextLayout layout = getTextLayout(text, font, g2);
        if (layout != null) {
    ...
    private static TextLayout getTextLayout(final String text, final Font font, final Graphics2D g2) {
    ...
    /**
     * Builds the circular shape and returns the result as an array of <code>Area</code>. Each <code>Area</code> is one of the bars composing the
     * shape.
     */
