...
public final class PanelXYPlot extends JPanel {
		...
		for (short nSerie = 0; nSerie < nbSerie; nSerie++) {
			...
			if (!Double.isNaN(yScale)) {
				...
				for (int nPoint = 0; nPoint < serie.getPointsCount(); nPoint++) {
					...
					int x1 = (int) ((serie.getPoints().get(nPoint).getX() - minXvalue) * xScale + PADDING + LABEL_PADDING);
					int y1 = (int) ((maxYvalue - serie.getPoints().get(nPoint).getY()) * yScale + PADDING);
	...
	private final void drawTitle(Graphics2D g2)
	{
		if (title != null) {
	...
	private final void drawXAxisLabel(Graphics2D g2)
	{
		if (xAxisLabel != null) {
	...
	private final void drawYAxisLabel(Graphics2D g2)
	{
	...
	private final void drawGridLines(Graphics2D g2)
	{
		int height = getHeight();
		int width = getWidth();
		FontMetrics metrics;
		...
		for (byte i = 0; i < NB_Y_DIV + 1; i++) {
			...
			int y0 = height - ((i * (height - PADDING * 2 - LABEL_PADDING)) / NB_Y_DIV + PADDING + LABEL_PADDING);
			int y1 = y0;
			if (getNbXPoints() > 0) {
		...
		for (byte i = 0; i < NB_X_DIV + 1; i++) {
			int x0 = i * (width - PADDING * 2 - LABEL_PADDING) / NB_X_DIV + PADDING + LABEL_PADDING;
			int x1 = x0;
			int y0 = height - PADDING - LABEL_PADDING;
	...
	private final void drawLineSeries(Graphics2D g2)
	{
		if (!listSeries.isEmpty()) {
			
			int nbSerie = listSeries.size();
			...
			for (short nSerie = 0; nSerie < nbSerie; nSerie++) {
				...
				for (short i = 0; i < listSeries.get(nSerie).size(); i++) {
					int xMark = listSeries.get(nSerie).get(i).x - POINT_WIDTH / 2;
					int yMark = listSeries.get(nSerie).get(i).y - POINT_WIDTH / 2;
	...
	public final void markPoint(Graphics g, Point p) {
		...
		if (clip != null && (p.x - 10 / 2) != clip.x | (p.y - 10 / 2) != clip.y) {
			repaint(clip);
		}
