...
//				{
//					for (int j = 0; j < listSeries.get(i).size(); j++)
//					{
//						if (Math.abs(listSeries.get(i).get(j).x - e.getX()) < 4 & Math.abs(listSeries.get(i).get(j).y - e.getY()) < 4) {
//
//							markPoint(getGraphics(), listSeries.get(i).get(j));
//							clip = getGraphics().getClipBounds(new Rectangle(listSeries.get(i).get(j).x - 10 / 2, listSeries.get(i).get(j).y - 10 / 2, 10, 10));
//						}
//					}
//				}
//			}
